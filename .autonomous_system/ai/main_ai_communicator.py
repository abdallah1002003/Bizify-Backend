...
"""
Main AI Communicator (Real Logic Edition)
Central hub for external AI interactions.
Transparently reports backend status.
"""
...
class MainAICommunicator:
    """
    Gateway to External Intelligence.
    """
    ...
    def __init__(self, project_root: Path):
        ...
        self.claude = ClaudeAPIClient(project_root)
        
    def query_model(self, prompt: str, model: str = "gemini") -> str:
        logger.info('Professional Grade Execution: Entering method')
        """
        Routes queries to the real backing model.
        """
        print(f"📡 Communicator: Routing query to {model}...")
        
        if model == "gemini":
            return self.gemini.generate_content(prompt)
        elif model == "claude":
            return self.claude.completion(prompt)
