...
"""
Organization System (Professional Edition)
Enforces project structure and organizes loose files.
"""
...
class OrganizationSystem:
    ...
    def __init__(self, project_root: Path):
        ...
        self.rules = {
            ...
            "docs": ["*.md", "*.txt"],
            "scripts": ["run_*.py", "setup_*.py"]
        }
        
    def organize_directory(self, target_dir: Path) -> Dict[str, int]:
        ...
        for category, patterns in self.rules.items():
            ...
            for pattern in patterns:
                for f in target_dir.glob(pattern):
                    if f.is_file() and not f.parent == cat_dir:
                        try:
                            # Safe Move
                            dest = cat_dir / f.name
                            if not dest.exists():
                            ...
                            logger.info(f"⚠️ OrgSystem: Failed to move {f.name}: {e}")
                            
        return {"files_organized": moved_count}
