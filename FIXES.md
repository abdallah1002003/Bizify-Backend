...
> **For teammates:** Every change made to the backend (`Bizify-Backend`) and AI service (`bizifyAI`) is recorded here, newest entries first.
...
Added a full chat session management router registered at prefix `/chat`.

**Endpoints:**
- `GET /api/v1/chat/sessions` — list all sessions for the current user, supports `?section_slug=` and `?idea_id=` filters
- `POST /api/v1/chat/sessions` — create a new session (`section_slug`, `idea_id`, `title` optional)
- `DELETE /api/v1/chat/sessions/{id}` — delete a session and all its messages
- `GET /api/v1/chat/sessions/{id}/messages` — get messages for a session (supports `?limit=N` for last N)
- `POST /api/v1/chat/sessions/{id}/messages` — save messages `{ messages: [{ role, content }] }`

**Notes:**
- Uses existing `ChatSession` and `ChatMessage` models (no migration needed)
- Section slug and title are stored in `conversation_summary_json` as `{ "section": "customers", "title": "...", "preview": "..." }`
- `flag_modified` is used on the JSON column to ensure SQLAlchemy detects the mutation on commit
- `_session_out` returns `idea_id` so the frontend can pass it back to section chat streams

### Modified: `app/api/api.py`
Registered the new `chat_sessions` router at prefix `/chat`.

---

## 2026-05-25 — Section Chat Stream `idea_id` Forwarding (AI Pipeline)

### Problem
When a user has multiple ideas, the section chatbots (customers, competition, etc.) were always using `current_idea_id` from the DB regardless of which idea page the user was on. This caused the chatbot to answer about the wrong idea.

### Fix: `app/api/v1/ai_pipeline.py`
All 9 section generate endpoints (`/customers`, `/competition`, `/market-potential`, `/idea-strategy`, `/business-model`, `/functions-list`, `/mvp-planning`, `/unit-economics`, `/go-to-market`) now accept an optional `idea_id` in the request body and forward it to BizifyAI so the correct idea is pinned before generating.

---

## 2026-05-25 — Section Generate Endpoints: Added POST Handler

### Problem
The 9 section GET endpoints existed but there were no POST handlers, causing 405 Method Not Allowed when the frontend tried to trigger per-section generation.

### Fix: `app/api/v1/ai_pipeline.py`
Added `POST` handlers to all 9 section endpoints. Each handler:
1. Extracts `idea_id` from the request body (optional)
2. Forwards it to BizifyAI so the correct idea is pinned (`crud.set_current_idea_id`)
3. Calls `_forward_post_to_ai` to trigger generation

**Sections fixed:** customers, competition, market-potential, idea-strategy, business-model, functions-list, mvp-planning, unit-economics, go-to-market

---

## 2026-05-25 — Regenerate / Regenerate-Custom Endpoints

### Problem
`regenerate_*` functions in BizifyAI called `generate_*(user_id, db)` positionally, but after adding `body: SectionRunInput` as the second parameter, `db` was passed as `body`, causing a type error.

### Fix (in BizifyAI, but triggered by this backend's forwarding)
All 9 `regenerate_*` functions were updated to use `db=db` keyword argument.
...
**Problem:**
`check_ai_usage` was applied to all AI routes including GETs (fetching cached section data). The dependency called `usage_repo.increment()` unconditionally, meaning reading your own previously-generated startup profile, customers, competition analysis etc. each burned one request from the user's monthly quota.
...
```
Max 500 rows per call, with proper `skip`/`limit` pagination.
...
| `app/api/v1/ai_pipeline.py` | Fixed 30 missing `user_id` injections; removed wrong `response_model` annotations; health/version to `_system_router`; usage increment moved here (POST success only); fixed duplicated ternary; added POST handlers for all 9 sections; added `idea_id` forwarding |
| `app/api/v1/chat_sessions.py` | **NEW** — full CRUD for chat sessions and messages |
| `app/api/api.py` | Registered `_system_router` for health/version endpoints; registered `chat_sessions` router at `/chat` |
| `app/api/dependencies.py` | Fixed `last_activity` None edge case; removed usage increment (moved to forwarding helper); GET requests skip quota |
...
| `app/models/ai/chat_session.py` | Added `GENERAL_BOT = "GENERAL_BOT"` to `SessionType` enum |
| `app/models/subscription.py` | Added `PENDING = "PENDING"` to `SubscriptionStatus` enum |
| `app/schemas/ai_pipeline.py` | Fixed 5 wrong response schema field names; expanded `AIPipelineStatusResponse` with all ready-flag fields |
...
| `alembic/versions/b2c3d4e5f6a1_add_pending_subscription_status.py` | **NEW** — adds `PENDING` to `subscriptionstatus` enum |
| `alembic/versions/c3d4e5f6a1b2_add_general_bot_session_type.py` | **NEW** — adds `GENERAL_BOT` to `sessiontype` enum |
...
| `db/crud.py` | Fixed `get_skills_from_profile()` — extracts `name` from skill dicts; fixed `"general_bot"` → `"GENERAL_BOT"`; fixed `role="user"` → `"USER"` and `role="assistant"` → `"AI"` |
| `routes/idea_intake.py` | Fixed both `_status` guards to use `decision == "needs_clarification"` |
| `routes/dependencies.py` | Added `idea_id: Optional[str]` to `SectionChatInput` |
| `routes/customers.py` + 8 other section routes | Added `crud.set_current_idea_id` call when `idea_id` is provided in chat stream |
| `agents/generalBot.py` | Added `_normalize_questionnaire()` + guard in `_run_new_user_pipeline_inline()` |
| `agents/OneProfileAnalysis.py` | Added type guard before `questionnaire.get()` call |
