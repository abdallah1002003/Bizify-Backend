...
class EmailWorkerService(BaseService):
    """
    Background worker service for processing the email queue.
                ...
                try:
                    if settings.MAIL_ENABLED:
                        await asyncio.wait_for(
                            self._send_fastapi_mail(email.to_email, email.subject, email.html_body),
=======
...
<<<<<<< HEAD
    ...
    async def run_email_worker(self, interval_seconds: int = 10) -> None:
        """Continuous background loop for processing emails."""
        logger.info("Email worker started (interval: %ds)", interval_seconds)
        while True:
            try:
                # Background workers need a fresh session for each loop
                async with AsyncSessionLocal() as db:
                    worker = EmailWorkerService(db)
                    await worker.process_email_queue()
