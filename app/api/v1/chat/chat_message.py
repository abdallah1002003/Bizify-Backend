from typing import List
from uuid import UUID
from fastapi import APIRouter, Depends, HTTPException
from app.core.pagination import LimitParam, SkipParam
from sqlalchemy.orm import Session
import app.models as models
from app.core.dependencies import get_current_active_user
from app.db.database import get_db
from app.schemas.chat.chat_message import ChatMessageCreate, ChatMessageUpdate, ChatMessageResponse
from app.services.chat import chat_service as service

router = APIRouter()


def _ensure_chat_session_owner(session: models.ChatSession, current_user: models.User) -> None:
    if session.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Access denied")


def _resolve_owned_session(
    db: Session,
    session_id: UUID,
    current_user: models.User,
) -> models.ChatSession:
    session = service.get_chat_session(db, id=session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="ChatSession not found")
    _ensure_chat_session_owner(session, current_user)
    return session


@router.get("/", response_model=List[ChatMessageResponse])
def read_chat_messages(  # type: ignore
    skip: SkipParam = 0,
    limit: LimitParam = 100,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user),
):
    return service.get_chat_messages(db, skip=skip, limit=limit, user_id=current_user.id)

@router.post("/", response_model=ChatMessageResponse)
def create_chat_message(  # type: ignore
    item_in: ChatMessageCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user),
):
    """Elite API: Appends a message to an active chat thread."""
    _resolve_owned_session(db, session_id=item_in.session_id, current_user=current_user)
    return service.add_message(
        db,
        session_id=item_in.session_id,
        role=item_in.role,
        content=item_in.content,
    )

@router.get("/{id}", response_model=ChatMessageResponse)
def read_chat_message(  # type: ignore
    id: UUID,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user),
):
    db_obj = service.get_chat_message(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ChatMessage not found")
    _resolve_owned_session(db, session_id=db_obj.session_id, current_user=current_user)
    return db_obj

@router.put("/{id}", response_model=ChatMessageResponse)
def update_chat_message(  # type: ignore
    id: UUID,
    item_in: ChatMessageUpdate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user),
):
    db_obj = service.get_chat_message(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ChatMessage not found")
    _resolve_owned_session(db, session_id=db_obj.session_id, current_user=current_user)
    data = item_in.model_dump(exclude_unset=True)
    data.pop("session_id", None)
    return service.update_chat_message(db, db_obj=db_obj, obj_in=data)

@router.delete("/{id}", response_model=ChatMessageResponse)
def delete_chat_message(  # type: ignore
    id: UUID,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user),
):
    db_obj = service.get_chat_message(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ChatMessage not found")
    _resolve_owned_session(db, session_id=db_obj.session_id, current_user=current_user)
    return service.delete_chat_message(db, id=id)
