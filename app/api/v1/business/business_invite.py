...
def read_business_invites(skip: SkipParam = 0, limit: LimitParam = 100, db: Session = Depends(get_db)):  # type: ignore
    return service.get_business_invites(db, skip=skip, limit=limit)

@router.post("/", response_model=BusinessInviteResponse)
def create_business_invite(item_in: BusinessInviteCreate, db: Session = Depends(get_db)):  # type: ignore
    return service.create_business_invite(db, obj_in=item_in)

@router.get("/{id}", response_model=BusinessInviteResponse)
def read_business_invite(id: UUID, db: Session = Depends(get_db)):  # type: ignore
...
def update_business_invite(id: UUID, item_in: BusinessInviteUpdate, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_business_invite(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="BusinessInvite not found")
...
def delete_business_invite(id: UUID, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_business_invite(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="BusinessInvite not found")
