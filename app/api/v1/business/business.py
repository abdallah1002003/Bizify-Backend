...
async def read_businesses(
    ...
    gen = await cache.get_generation_key(f"businesses:{current_user.id}")
    ...
    result = await service.get_businesses(skip=skip, limit=limit, owner_id=current_user.id)
...
async def create_business(
    ...
    result = await service.create_business(obj_in=item_in)
    
    await get_cache_manager().increment_generation_key(f"businesses:{current_user.id}")
    return result
...
async def read_business(
    ...
    db_obj = await service.get_business(id=id)
    if not db_obj:
...
async def update_business(
    ...
    result = await service.update_business(db_obj=db_obj, obj_in=item_in)
    
    cache = get_cache_manager()
    await cache.increment_generation_key(f"businesses:{current_user.id}")
    await cache.delete(f"api:business:{id}:user:{current_user.id}")
    return result
...
async def delete_business(
    ...
    result = await service.delete_business(id=id)
