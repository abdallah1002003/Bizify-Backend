...
def read_business_invite_ideas(skip: SkipParam = 0, limit: LimitParam = 100, db: Session = Depends(get_db)):  # type: ignore
    return service.get_business_invite_ideas(db, skip=skip, limit=limit)

@router.post("/", response_model=BusinessInviteIdeaResponse)
def create_business_invite_idea(item_in: BusinessInviteIdeaCreate, db: Session = Depends(get_db)):  # type: ignore
    return service.create_business_invite_idea(db, obj_in=item_in)

@router.get("/{id}", response_model=BusinessInviteIdeaResponse)
def read_business_invite_idea(id: UUID, db: Session = Depends(get_db)):  # type: ignore
...
def update_business_invite_idea(id: UUID, item_in: BusinessInviteIdeaUpdate, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_business_invite_idea(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="BusinessInviteIdea not found")
...
def delete_business_invite_idea(id: UUID, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_business_invite_idea(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="BusinessInviteIdea not found")
