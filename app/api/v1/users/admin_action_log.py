...
async def read_admin_action_logs(
    ...
    return await service.get_admin_action_logs(skip=skip, limit=limit)
...
async def create_admin_action_log(
    ...
    return await service.create_admin_action_log(obj_in=item_in)


@router.get("/{id}", response_model=AdminActionLogResponse)
async def read_admin_action_log(
    id: UUID, 
    service: UserService = Depends(get_user_service)
):
    db_obj = await service.get_admin_action_log(id=id)
    if not db_obj:
...
async def update_admin_action_log(
    id: UUID, 
    ...
    if not db_obj:
        raise HTTPException(status_code=404, detail="AdminActionLog not found")
    return await service.update_admin_action_log(db_obj=db_obj, obj_in=item_in)


@router.delete("/{id}", response_model=AdminActionLogResponse)
async def delete_admin_action_log(
    id: UUID, 
    service: UserService = Depends(get_user_service)
):
    db_obj = await service.get_admin_action_log(id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="AdminActionLog not found")
    return await service.delete_admin_action_log(id=id)
