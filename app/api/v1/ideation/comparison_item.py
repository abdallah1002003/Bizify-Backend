...
def read_comparison_items(skip: SkipParam = 0, limit: LimitParam = 100, db: Session = Depends(get_db)):  # type: ignore
    return service.get_comparison_items(db, skip=skip, limit=limit)

@router.post("/", response_model=ComparisonItemResponse)
def create_comparison_item(item_in: ComparisonItemCreate, db: Session = Depends(get_db)):  # type: ignore
    return service.create_comparison_item(db, obj_in=item_in)

@router.get("/{id}", response_model=ComparisonItemResponse)
def read_comparison_item(id: UUID, db: Session = Depends(get_db)):  # type: ignore
...
def update_comparison_item(id: UUID, item_in: ComparisonItemUpdate, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_comparison_item(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ComparisonItem not found")
...
def delete_comparison_item(id: UUID, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_comparison_item(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ComparisonItem not found")
