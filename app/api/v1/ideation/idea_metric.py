...
def read_idea_metrics(skip: SkipParam = 0, limit: LimitParam = 100, db: Session = Depends(get_db)):
    return service.get_idea_metrics(db, skip=skip, limit=limit)

@router.post("/", response_model=IdeaMetricResponse)
def create_idea_metric(item_in: IdeaMetricCreate, db: Session = Depends(get_db)):
    return service.create_idea_metric(db, obj_in=item_in)

@router.get("/{id}", response_model=IdeaMetricResponse)
def read_idea_metric(id: UUID, db: Session = Depends(get_db)):
...
def update_idea_metric(id: UUID, item_in: IdeaMetricUpdate, db: Session = Depends(get_db)):
    db_obj = service.get_idea_metric(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="IdeaMetric not found")
...
def delete_idea_metric(id: UUID, db: Session = Depends(get_db)):
    db_obj = service.get_idea_metric(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="IdeaMetric not found")
