...
@limiter.limit("5/minute")
def forgot_password(request: Request, data: OTPResendRequest, db: Session = Depends(get_db)) -> dict[str, str]:
...
@limiter.limit("10/minute")
def verify_reset_code(request: Request, data: OTPVerify, db: Session = Depends(get_db)) -> dict[str, str]:
...
def test_email(
    ...
    """
    Admin-only diagnostic endpoint for email delivery. Returns the configured
    provider, a config-validation summary, and (if reachable) the result of an
    actual send. Restricted to admins because it triggers a real email send to
    an arbitrary address and would otherwise be an email-bombing vector.
    """
