...
def create_embedding(item_in: EmbeddingCreate, db: Session = Depends(get_db)):
    return service.create_embedding(db, obj_in=item_in)

@router.get("/{id}", response_model=EmbeddingResponse)
def read_embedding(id: UUID, db: Session = Depends(get_db)):
...
def update_embedding(id: UUID, item_in: EmbeddingUpdate, db: Session = Depends(get_db)):
    db_obj = service.get_embedding(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="Embedding not found")
...
def delete_embedding(id: UUID, db: Session = Depends(get_db)):
    db_obj = service.get_embedding(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="Embedding not found")
