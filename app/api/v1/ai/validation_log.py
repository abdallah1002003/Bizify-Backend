...
def read_validation_logs(skip: SkipParam = 0, limit: LimitParam = 100, db: Session = Depends(get_db)):  # type: ignore
    return service.get_validation_logs(db, skip=skip, limit=limit)

@router.post("/", response_model=ValidationLogResponse)
def create_validation_log(item_in: ValidationLogCreate, db: Session = Depends(get_db)):  # type: ignore
...
def read_validation_log(id: UUID, db: Session = Depends(get_db)):  # type: ignore
...
def update_validation_log(id: UUID, item_in: ValidationLogUpdate, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_validation_log(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ValidationLog not found")
...
def delete_validation_log(id: UUID, db: Session = Depends(get_db)):  # type: ignore
    db_obj = service.get_validation_log(db, id=id)
    if not db_obj:
        raise HTTPException(status_code=404, detail="ValidationLog not found")
